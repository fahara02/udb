...
// UDB vendoring adjustment: this file was originally the crate root
// (`lib.rs`) where `crate::protocol::*` resolved to the sibling
// `protocol.rs`. Vendored as a submodule of UDB, `crate` now points
// at the udb crate root, so we use `self::protocol::*` instead. The
// only file-local change vs. the upstream rev — see NOTICE.md.
