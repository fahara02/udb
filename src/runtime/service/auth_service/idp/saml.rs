...
<md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata" entityID="https://idp.example.com/meta">
  ...
  <saml:Assertion ID="{id}">
    ...
    // A fixed RSA-2048 keypair + self-signed cert (generated once with openssl)
    // so the test is deterministic and fast. The private key signs a SignedInfo
    // whose DigestValue is the SHA-256 of the *canonical* (exclusive-C14N,
    // enveloped-signature-stripped) Assertion; the cert is what `verify_signature`
    // checks against. This proves the C14N digest path is exercised end-to-end.
...
-----END PRIVATE KEY-----\n";
    ...
    fn signed_response(name_id: &str) -> String {
        ...
        // Assertion WITHOUT its signature (the to-be-signed content). Note the
        // self-closed <ds:Signature/> placeholder is what the enveloped
        // transform strips; we sign the canonical form with no Signature.
        let response = format!(
        ...
        // 1. Canonicalize the Assertion with the enveloped signature stripped and
        //    compute its SHA-256 digest (the DigestValue).
        let no_sig = response.replace("__SIG__", "");
        let canon_assertion =
            c14n::canonicalize_element_by_id(&no_sig, assertion_id, &[]).expect("c14n assertion");
        let digest = Sha256::digest(&canon_assertion);
        let digest_b64 = base64::engine::general_purpose::STANDARD.encode(digest);

        // 2. Build SignedInfo referencing the Assertion with that digest.
        ...
        // 3. Canonicalize SignedInfo and RSA-PKCS1v15 sign over it (SHA-256),
        //    matching verify_signature's unprefixed DigestInfo construction.
        let wrapped =
            format!(r#"<wrap xmlns:ds="http://www.w3.org/2000/09/xmldsig#">{signed_info}</wrap>"#);
        let canon_si = c14n::canonicalize_element(&wrapped, "SignedInfo", &[]).expect("c14n si");
        const SHA256_DIGEST_INFO_PREFIX: [u8; 19] = [
            0x30, 0x31, 0x30, 0x0d, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02,
            0x01, 0x05, 0x00, 0x04, 0x20,
        ];
        ...
        let key = rsa::RsaPrivateKey::from_pkcs8_pem(TEST_KEY_PKCS8_PEM).expect("load key");
        let sig = key
        ...
        response.replace("__SIG__", &signature)
    }
