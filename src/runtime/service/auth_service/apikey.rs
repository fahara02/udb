...
impl ApiKeyServiceImpl {
    ...
    /// Whether the caller holds genuine cross-tenant/project platform authority.
    /// Reuse the method-security predicate so API-key handlers cannot interpret a
    /// project-bound broad admin scope as permission to escape that project.
    /// Empty caller / no recognized authority → deny by default.
    fn caller_is_cross_tenant_admin(context: Option<&common_pb::RequestContext>) -> bool {
        let Some(context) = context else {
            return false;
        };
        let (tenant_id, project_id) = context
            .tenant
            .as_ref()
            .map(|tenant| (tenant.tenant_id.as_str(), tenant.project_id.as_str()))
            .unwrap_or(("", ""));
        crate::runtime::service::method_security::has_cross_tenant_platform_authority(
            tenant_id,
            project_id,
            &context.scopes,
            &context.roles,
...
impl ApiKeyService for ApiKeyServiceImpl {
    async fn create_api_key(
        ...
        // Reject an explicitly foreign body scope before owner/grant lookup. The
        // body is not authoritative, so this is only a preflight; the resolved
        // grant is checked again below before any key material is persisted.
        if !requested_tenant.trim().is_empty() || !requested_project.trim().is_empty() {
...
mod tests {
    ...
    fn ctx_project(tenant: &str, project: &str, roles: &[&str]) -> RequestContext {
        ctx_project_with_scopes(tenant, project, &[], roles)
    }

    fn ctx_project_with_scopes(
        tenant: &str,
        project: &str,
        scopes: &[&str],
        roles: &[&str],
    ...
    #[tokio::test]
    async fn create_api_key_denies_bound_broad_scope_before_owner_lookup() {
        let svc = svc_with_config_records(config_with_api_key_hash_secret(), Vec::new());
        ...
        let err = crate::runtime::service::method_security::scope_claim_context_for_test(
            ...
            async {
                svc.create_api_key(Request::new(apikey_pb::CreateApiKeyRequest {
                    owner_id: "foreign-service-account".to_string(),
                    context: Some(ctx_project("acme", "project-b", &[])),
                    ..Default::default()
                }))
                .await
            },
        ...
        .await
        .expect_err("bound broad admin must be denied before foreign owner lookup");
        assert_policy_detail(
            &err,
            "api_key_project_scope",
            "caller_project_mismatch",
