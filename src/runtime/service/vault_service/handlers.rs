...
use super::config::{
    ...
    TOPIC_DB_CREDENTIAL_ISSUED, VAULT_DB_CREDENTIAL_LEASE_MSG, VAULT_ED25519_PREFIX,
    VAULT_HMAC_PREFIX, VAULT_SECRET_MSG, VAULT_TRANSIT_KEY_MSG, max_batch_decrypt_items,
...
use super::errors::{
    ...
    vault_db_role_creation_status, vault_field_violation, vault_internal_status,
...
pub(crate) async fn generate_database_credentials(
    ...
    let mut authority_tx = pool.begin().await.map_err(|err| {
        vault_db_role_creation_status(format!(
            "vault database credential authority transaction failed: {err}"
        ))
    })?;
    let provenance = create_postgres_login_role(
        authority_tx.as_mut(),
        &username,
        &password,
        expires_at,
    ...
    authority_tx.commit().await.map_err(|err| {
        vault_db_role_creation_status(format!(
            "vault database credential authority commit failed: {err}"
        ))
    })?;
    ...
    svc.emit(
        ...
        serde_json::json!({
            ...
            "username": &username,
            "project_id": &context.project_id,
            "target_instance": &provenance.target_instance,
    ...
    Ok(Response::new(
        vault_pb::GenerateDatabaseCredentialsResponse {
            ...
            message: "tenant/project-bound read-only database credentials issued".to_string(),
            error: None,
        },
    ))
}
