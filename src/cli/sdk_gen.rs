...
fn sdk_diff(selector: &SdkSelector, against: &str) -> i32 {
    ...
    // Name a generator-version mismatch explicitly. The content comparison below
    // does catch it — the version is in the header — but it reports it as
    // "STALE (0 of N entities drifted)", which reads like a false alarm and was
    // understood as "the check does not verify the generator version". Say what
    // actually differs, the way the broker refuses a mismatched
    // UDB_EXPECTED_VERSION.
    if let Some(stamped) = committed_generator_version(&committed_normalized)
        && stamped != UDB_PACKAGE_VERSION
    {
        eprintln!(
            "sdk diff: {against} was generated by udb {stamped}, this CLI is udb {UDB_PACKAGE_VERSION}. \
             Regenerate with `udb sdk generate --project-proto {proto_root} --lang go` and commit. \
             A file produced by a superseded toolchain can be byte-identical in the parts \
             you read and still miss fixes made since."
        );
        return 1;
    }
...
/// The `udb <version>` stamp from a generated file's header, if present.
///
/// The header line is `// udb 0.5.16 · project-proto manifest sha256: …`.
fn committed_generator_version(text: &str) -> Option<String> {
    text.lines()
        .take(10)
        .find_map(|line| line.trim_start().strip_prefix("// udb "))
...
/// entities. Emits, per entity, `<Entity>ToUDBRecord(*T) (map[string]any, error)` and
...
fn render_go_entities_file(entities: &[EntityDescriptor], package: &str) -> Result<String, String> {
    ...
    let mut needs_json_message = false;
    ...
    if !proto_imports.is_empty() || needs_timestamppb || needs_json_message {
        out.push('\n');
    }
    if needs_json_message {
        out.push_str("\t\"google.golang.org/protobuf/encoding/protojson\"\n");
        out.push_str("\t\"google.golang.org/protobuf/proto\"\n");
        out.push_str("\t\"google.golang.org/protobuf/reflect/protoreflect\"\n");
    }
    ...
    for (entity, alias, type_name) in &renderable {
        ...
        out.push_str(&format!(
            "// {name}ToUDBRecord converts a *{qualified} into a UDB record. It returns\n\
             // an error when a column cannot be encoded (a message-valued JSON\n\
             // column whose protojson encoding fails), so a write never silently\n\
             // drops a field.\n\
             func {name}ToUDBRecord(m *{qualified}) (map[string]any, error) {{\n\
             \tif m == nil {{\n\t\treturn nil, nil\n\t}}\n\tr := map[string]any{{}}\n",
...
/// `*pb.Message`, so those arms emitted `json.RawMessage(m.GetX())` (not a
/// legal conversion) and `m.X = encoded` (a string into a message pointer):
/// six type errors per entity, and a package that does not compile.
///
/// Arrays, well-known timestamps and enums are matched by arms that run BEFORE
/// the JSON arm, so an `Unknown` scalar kind reaching here is a message. An
/// enum only lands here if its declaration was never parsed; the emitted
/// helper then fails closed at runtime rather than mis-decoding.
fn is_message_json_column(column: &EntityColumnDescriptor) -> bool {
    (column.is_json || column.is_jsonb)
        && !column.is_array
        && !is_go_timestamp(&column.proto_type)
        && column.enum_values.is_empty()
        // A proto `map<K,V>` is ALSO an `Unknown` scalar kind, and it is NOT a
        // message: its Go form is `map[K]V`, which does not implement
        // proto.Message. Routing it to the message arm produced Go that gofmt
        // accepted and `go build` rejected - a class nothing caught until the
        // generator's output was actually compiled.
        && go_map_types(&column.proto_type).is_none()
        && !column.proto_type.trim_start_matches('.').starts_with("map<")
        && matches!(go_scalar_kind(&column.proto_type), GoScalar::Unknown)
}

/// Go type name for a proto scalar, or `None` for anything non-scalar.
fn go_scalar_type_name(proto_type: &str) -> Option<&'static str> {
    Some(match go_scalar_kind(proto_type) {
        GoScalar::String => "string",
        GoScalar::Bool => "bool",
        GoScalar::Int32 => "int32",
        GoScalar::Int64 => "int64",
        GoScalar::Uint32 => "uint32",
        GoScalar::Uint64 => "uint64",
        GoScalar::Float32 => "float32",
        GoScalar::Float64 => "float64",
        GoScalar::Bytes => "[]byte",
        GoScalar::Unknown => return None,
    })
}

/// The Go map type for a proto `map<K,V>` column whose key AND value are both
/// scalars, e.g. `map<string, string>` -> `map[string]string`.
///
...
fn go_map_types(proto_type: &str) -> Option<String> {
    ...
    let key = go_scalar_type_name(k.trim())?;
    let value = go_scalar_type_name(v.trim())?;
    // A []byte key is not valid Go map key syntax; protobuf forbids it too.
    if key == "[]byte" {
        return None;
    }
...
/// A map column stored in a JSON/JSONB column, with a Go map form.
fn is_map_json_column(column: &EntityColumnDescriptor) -> bool {
    (column.is_json || column.is_jsonb)
        && !column.is_array
...
fn go_to_record_stmt(column: &EntityColumnDescriptor) -> String {
    ...
    if column.is_json || column.is_jsonb {
        // A message goes through protojson, not the text arms below. Message
        // fields are pointers in Go regardless of `optional`, so nil is always
        // the "unset" test; unset is omitted so the column keeps its SQL NULL.
        if is_map_json_column(column) {
            // A Go map marshals with encoding/json directly - it is not a
            // proto.Message and must not reach the protojson helper.
            return format!(
                "\tif m.{field} != nil {{\n\t\tv, err := json.Marshal(m.{getter}())\n\t\tif err != nil {{\n\t\t\treturn nil, fmt.Errorf(\"encode column %q: %w\", \"{key}\", err)\n\t\t}}\n\t\tr[\"{key}\"] = json.RawMessage(v)\n\t}}\n"
            );
        }
        if is_message_json_column(column) {
            return format!(
                "\tif m.{field} != nil {{\n\t\tv, err := udbEncodeJSONMessage(m.{getter}())\n\t\tif err != nil {{\n\t\t\treturn nil, fmt.Errorf(\"encode column %q: %w\", \"{key}\", err)\n\t\t}}\n\t\tr[\"{key}\"] = v\n\t}}\n"
            );
        }
        if column.has_presence {
            return format!(
...
fn go_from_row_stmt(column: &EntityColumnDescriptor, alias: &str) -> String {
    ...
    if column.is_json || column.is_jsonb {
        // A message is decoded into a freshly allocated message of the field's
        // own type, resolved through protoreflect - so the generated file never
        // has to name that type or import its package.
        if let Some(go_map) = go_map_types(&column.proto_type)
            && is_map_json_column(column)
        {
            // Symmetric with the write arm: decode straight into the Go map.
            return format!(
        ...
        if is_message_json_column(column) {
            return format!(
                "\tif raw, ok := row[\"{key}\"]; ok && raw != nil {{\n\t\tif err := udbDecodeJSONMessage(m, \"{key}\", raw); err != nil {{\n{fail}\t\t}}\n\t}}\n",
                fail = fail("\t\t\t"),
            );
        }
...
fn go_coercion_helpers(needs_time: bool, needs_json_message: bool) -> String {
    let mut out = String::new();
    out.push_str(
...
#[cfg(test)]
mod tests {
    use super::{UDB_PACKAGE_VERSION, committed_generator_version};

    /// The header stamp is what tells an operator their generated file came from
    /// a superseded toolchain. A deployment ran generated code stamped udb 0.4.37
    /// against a 0.5.14 broker without anything saying so.
    #[test]
    fn generator_version_is_read_from_the_header() {
        let mut header = String::new();
        header.push_str("// Code generated by `udb sdk generate --project-proto`. DO NOT EDIT.\n");
        header.push_str("//\n");
        header.push_str("// udb 0.4.37 * project-proto manifest sha256: abc123\n");
        header.push_str("//\n");
        header.push_str("package udbgen\n");
        assert_eq!(
            committed_generator_version(&header).as_deref(),
            Some("0.4.37")
        );
        assert_ne!(
            committed_generator_version(&header).as_deref(),
            Some(UDB_PACKAGE_VERSION),
            "a 0.4.37 artifact must not compare equal to this CLI"
        );
    }

    #[test]
    fn a_file_without_a_stamp_is_not_mistaken_for_a_match() {
        assert_eq!(committed_generator_version("package udbgen\n"), None);
        // The stamp must come from the header, not from prose further down.
        let mut late = String::from("package udbgen\n");
        for _ in 0..20 {
            late.push_str("// filler\n");
        }
        late.push_str("// udb 9.9.9\n");
        assert_eq!(committed_generator_version(&late), None);
    }
    ...
    fn bytes_column_base64_decodes_on_read() {
        ...
        let helpers = go_coercion_helpers(false, false);
        assert!(
    ...
    // A protobuf MESSAGE stored in a JSON/JSONB column. The JSON arm assumed the
    // getter returns text, so it emitted `json.RawMessage(m.GetAuditInfo())` on a
    // `*commonv1.AuditInfo` and `m.AuditInfo = encoded` from a string — six type
    ...
    fn message_typed_json_column_does_not_emit_uncompilable_go() {
        ...
        let write = go_to_record_stmt(&col);
        assert!(
            !write.contains("json.RawMessage(m.GetAuditInfo())"),
            "a message getter is not convertible to json.RawMessage, got: {write}"
        );
        assert!(
            write.contains("v, err := udbEncodeJSONMessage(m.GetAuditInfo())")
                && write.contains("if m.AuditInfo != nil {"),
            "a message column must marshal through protojson, got: {write}"
        );
        assert!(
            write.contains("encode column %q: %w"),
            "a marshal failure must fail the write, not drop the column: {write}"
        );

        let read = go_from_row_stmt(&col, "acmev1");
        assert!(
            !read.contains("m.AuditInfo = encoded"),
            "a string cannot be assigned to a message field, got: {read}"
        );
        assert!(
            read.contains("udbDecodeJSONMessage(m, \"audit_info\", raw)")
                && read.contains("decode column %q: %w"),
            "a message column must decode through protojson and fail closed: {read}"
        );
    }

    /// 0.5.18 shipped a generator that could not emit Go AT ALL for any proto set
    /// containing a message-in-JSON column: the helper block was written with
    /// `push_str` but carried `format!`-style `{{`/`}}` escapes, so the generated
    /// file literally contained `{{` and gofmt rejected it.
    ///
    /// The statement-level tests could not catch it because they only exercise
    /// `go_to_record_stmt`/`go_from_row_stmt`, and the scaffold compile job could
    ...
    fn json_message_helpers_emit_single_braces() {
        ...
        assert!(
            helpers.contains("func udbEncodeJSONMessage("),
            "the message helpers must be emitted when requested"
        );
        assert!(
            !helpers.contains("{{") && !helpers.contains("}}"),
            "emitted Go must not contain doubled braces: push_str does NOT process \
             format! escapes, so a doubled brace reaches the file verbatim and gofmt \
             rejects it. Got:\n{helpers}"
        );
        // Balanced braces is the property gofmt was checking for us.
        assert_eq!(
            helpers.matches('{').count(),
            helpers.matches('}').count(),
            "emitted helper braces must balance"
        );
        // And it must not be emitted when no message-in-JSON column exists.
        let without = go_coercion_helpers(false, false);
        assert!(!without.contains("udbEncodeJSONMessage"));
    }

    // The message arm must not swallow the neighbouring cases: an enum column is
    // matched by the enum arm above it, and a plain string JSON column keeps the
    // json.RawMessage text form it has always had.
    #[test]
    fn json_message_arm_does_not_capture_enums_or_text() {
        ...
        assert!(
            ...
            "an enum column must stay on the enum arm"
        );

        let mut text = column("metadata", "string");
        text.is_jsonb = true;
        assert!(!is_message_json_column(&text));
        assert!(
            go_to_record_stmt(&text).contains("json.RawMessage(m.GetMetadata())"),
        ...
        assert!(!is_message_json_column(&repeated));

        // A well-known timestamp in a JSON column stays on the timestamp arm.
        let mut ts = column("seen_at", ".google.protobuf.Timestamp");
        ts.is_jsonb = true;
        assert!(!is_message_json_column(&ts));
    }
