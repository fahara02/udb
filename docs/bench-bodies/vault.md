...
| [ ] | GetTransitPublicKey | READ_ONLY | GetTransitPublicKeyRequest | `{ "tenant_id": "<seed:tenant_id>", "key_name": "<seed:vault_key_name>" }` | exports the ed25519 public key(s) for external signature verification; requires a seeded key created with the ed25519 signing algorithm (non-ed25519 keys are rejected). |
