...
| [ ] | StartWorkflow | MUTATION | StartWorkflowRequest | `{ "tenant_id": "<seed:tenant_id>", "project_id": "", "workflow_type": "sdk.perf.workflow", "total_steps": 20, "payload": "{}", "compensations": "[]", "correlation_id": "<seed:record_id>" }` | starts a bounded multi-step workflow tied to the seeded record. `project_id` is empty because the workflow schema treats non-empty project ids as UUIDs; the default live project code is not a UUID. |
