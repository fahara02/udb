...
| [ ] | RegisterAsset | MUTATION | RegisterAssetRequest | `{ "tenant_id": "<seed:tenant_id>", "project_id": "", "file_id": "<seed:file_id>", "name": "logo.png", "media_type": "image/png", "metadata": "{\"source\":\"upload\"}" }` | `file_id` references an existing StorageService file (`<seed:file_id>`). `project_id` is empty because the native asset schema treats non-empty project ids as UUIDs; the default live project code is not a UUID. Returns `asset_id` — capture as `<seed:asset_id>`. |
