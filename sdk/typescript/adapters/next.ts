...
/** A WHATWG `Headers`-like object (App Router `request.headers`). */
export interface HeadersLike {
  get(name: string): string | null;
  forEach(cb: (value: string, key: string) => void): void;
}

/** Anything carrying `headers` (a Next request), or the header source directly. */
export type NextRequestLike =
  | { headers: HeadersLike | HeaderBag }
  | HeadersLike
  | HeaderBag;

function isHeadersLike(value: any): value is HeadersLike {
...
export function toHeaderBag(source: NextRequestLike | undefined): HeaderBag {
  if (source == null) return {};
  // A Next request object: unwrap its `.headers`.
  const headers: any =
    "headers" in (source as any) && (source as any).headers != null
 ...
 * ```
 */
export function udbContext(
  reqOrHeaders: NextRequestLike | undefined,
  options: UdbAdapterOptions,
): UdbRequestContext {
  return contextFromHeaders(toHeaderBag(reqOrHeaders), options);
}
 ...
 * ```
 */
export function withUdb<T>(
  options: UdbAdapterOptions,
  handler: (ctx: UdbRequestContext) => Promise<T> | T,
): (reqOrHeaders?: NextRequestLike) => Promise<T> {
  return async (reqOrHeaders?: NextRequestLike): Promise<T> => {
