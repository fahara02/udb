...
# dup-key fails), bootstrap, pre-seed saga/DLQ rows scoped to THIS run's tenant UUID,
# restart broker on isolated ports, run TestLivePerf, print the failure set.
$ROOT = "E:\Projects\udb"
...
Write-Host "== pre-seed saga + dlq rows (tenant $tid) ==" -ForegroundColor Cyan
$sql = @"
...
"@
# NOTE: udb_system tables are created by the broker at first boot, so seed AFTER the
# broker has started once. We start the broker, wait for ready, THEN seed, then run.
...
& go test ./udbclient/ -run '^TestLivePerf$' -count=1 -v -timeout 15m 2>&1 | Tee-Object -FilePath "$ROOT\.bench-local\grind-run.log" | Out-Null
