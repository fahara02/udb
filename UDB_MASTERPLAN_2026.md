...
| **3.2** Vector-store lease | BLOCKED | **Full-canonical — build real multi-process CAS** (projection-pin rejected, non-negotiable) | ✅ DONE for current contract — Elasticsearch native CAS observed green; Qdrant current image fails closed; Weaviate/Pinecone terminally fail closed because no CAS primitive exists |
